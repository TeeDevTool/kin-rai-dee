...
import { Cuisine, SWEET_CUISINES } from "@/app/[locale]/stores/filterStore";
...
export function SweetFilters() {
  ...
  const { cuisines, setCuisines } = useFilterStore((state) => state);

  const isAllSelected = SWEET_CUISINES.every((c) => cuisines.includes(c));

  const toggleAll = () => {
    if (isAllSelected) {
      SWEET_CUISINES.forEach((c) => {
        if (cuisines.includes(c)) setCuisines(c);
      });
    } else {
      SWEET_CUISINES.forEach((c) => {
        if (!cuisines.includes(c)) setCuisines(c);
  ...
  return (
    <div className="mb-8 grid w-full gap-6 xl:mb-12">
      <div className="grid gap-3">
        <h4 className="text-subtitle1 w-fit text-gray-700">
          {t("sweet_cuisine_header")}
        </h4>
        <div className="flex flex-wrap gap-3">
          <Button
            ...
            onClick={toggleAll}
          >
            <CheckIcon active={isAllSelected} />
            {t("all")}
          </Button>
          {SWEET_CUISINE_FILTERS.map((cuisine) => {
            ...
            return (
              <Button
                ...
                variant="outline"
                active={isActive}
                onClick={() => setCuisines(cuisine.id)}
              >
