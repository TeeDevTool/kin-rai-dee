...
export default function LocaleController() {
  ...
  function localeChange(nextLocale: string) {
    startTransition(() => {
      ...
      router.replace(`${nextPath}?${searchParams.toString()}`, {
        scroll: false,
      });
    });
  }
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          className="text-primary fixed top-8 right-8"
          variant="outline"
          size="icon"
        >
          <svg
